...
- 日本語のBlocklyエディタ
- 5つのミッションと約15分の入門チュートリアル
- 離陸、着陸、移動、回転、写真、待機、繰り返し、メッセージブロック
- バージョン付きプロジェクトのJSON保存・読み込み
- カメラON/OFF、ライブ表示、撮影ブロックによるJPEG保存とスナップショット一覧（録画なし）
...
- 小学生と監督者による授業評価、実機のオフライン回帰確認

M3〜M5のソフトウェア実装を追加しました。実装範囲と残る受入確認は[PLAN.md](PLAN.md)、カメラ・配布・署名の手順は[先生向けガイド](docs/teacher-guide.md)を参照してください。

## 実機で起動する

Tello EDUを事前に以下のステーションモード（Wi-Fiのクライアントモード）へ設定し、DHCP予約などでIPを確認してください。
PCを同じネットワークに接続し、UDP 8890と映像用11111の受信を許可します。
...
離陸→移動→着陸をシミュレーションし、先生が安全確認にチェックして「実機で飛ばす」を押します。写真を使うときは「写真の保存先」を選択します。「カメラON」でライブ表示し、撮影ブロックでJPEGを保存して画面に表示します。カメラOFFでも撮影時には自動起動します。動画ファイルは作りません。
