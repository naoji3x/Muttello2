...
- 飛行コマンドを1件ずつ実行し、15秒の応答タイムアウトや接続断で停止。飛行コマンドの自動再送なし
...
「ドローンに接続する」は、SDK初期化の `command` に応答がない場合、各回5秒待ち、1秒の間隔を置いて最大3回接続を試します。途中でボタンを押し直す必要はありません。各回、両方のUDPソケットの解放を待ち、受信データを消してソケットを再作成します。SDKの成功応答と新しい状態データの両方を確認して接続完了とします。
機体の拒否応答・UDP初期化エラー・状態データ未受信では自動再試行しません。初回接続の失敗や、飛行操作前の接続断では、原因を確認し、機体を地面に置いてボタンから再度接続できます。飛行コマンドとAP設定ヘルパーの自動再送は行いません。
正常な着陸応答を受信した後に機体の電源を切った場合も、電源を入れ直して同じIPでルーターへ接続した後、ボタンから再接続できます。
飛行操作のコマンド送信後、着陸の成功を確認する前に通信が不確実になった場合は追加の通常コマンドを送らず、再接続も拒否します。先生が機体の着陸・状態を確認してからアプリを再起動してください。
